...
export default function ExecutiveModal({ executive, isCurrent, isOpen, onClose }: ExecutiveModalProps) {
        ...
        <div className="flex flex-col max-h-[75vh] overflow-y-auto">
          ...
          <div className="w-full relative">
            ...
            <div className="relative h-80">
              ...
              {!imageError ? (
                <Image
                  src={executive.image || "/placeholder.svg?height=600&width=400"}
                  alt={executive.name}
                  fill
              ...
              <div className="absolute bottom-6 left-6 right-6 z-20">
                <h2 className="text-xl font-bold text-white mb-2">{executive.name}</h2>
                <p className="text-blue-200 text-lg font-medium">{executive.position}</p>
                <div className="flex items-center gap-2 mt-2">
                  <Calendar className="w-4 h-4 text-blue-200" />
                  <span className="text-blue-200 text-sm">{isCurrent ? "Current Term" : executive.term}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-8">
              ...
              {executive.bio && (
                <div>
                  ...
                  <p className="text-sm text-slate-700 leading-relaxed">{executive.bio}</p>
                </div>
              )}
                  ...
                  {executive.email && (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-slate-600 text-xs">{executive.email}</p>
                      </div>
                  ...
                  {executive.phone && (
                    <div className="flex items-center gap-3 py-4 px-2 bg-slate-50 rounded-lg">
                      ...
                      <div>
                        ...
                        <p className="text-slate-600 text-xs">{executive.phone}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {executive.location && (
                    <div className="flex items-center gap-3 py-4 px-2 bg-slate-50 rounded-lg">
                      ...
                      <div>
                        ...
                        <p className="text-slate-600 text-xs">{executive.location}</p>
                      </div>
                    </div>
                  )}
              ...
              <div className="bg-blue-50 py-4 px-2 rounded-xl">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-blue-900 text-xs">{executive.position}</p>
                  </div>
                  <div>
                    <p className="text-sm text-blue-700 font-medium">Term Period</p>
                    <p className="text-blue-900 text-xs">{isCurrent ? "2023-2025 (Current)" : executive.term}</p>
                  </div>
